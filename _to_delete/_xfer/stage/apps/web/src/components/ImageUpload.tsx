 ...
 * true.
 *
 * The bytes never touch our API (CLAUDE.md §4): the server signs a POST scoped
 * to one key, one type and one size, and the browser submits to storage
 * directly. Everything here is the wrapper around `uploadDirect`.
 *
 * The type check below is a courtesy to the reader, not a control. The server
 * re-checks against its own allowlist and R2 enforces the size through the
 * policy — a client that skipped this entirely would still be refused.
 */
