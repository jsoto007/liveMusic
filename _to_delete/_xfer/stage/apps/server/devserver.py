...
def _presigned_post(key, content_type, max_bytes, expires_in=None):
    return {
        "url": f"http://localhost:{PORT}/__storage",
        "fields": {"key": key, "Content-Type": content_type, "max_bytes": str(max_bytes)},
    }
...
@app.post("/__storage")
def storage_put():
    """Stand-in for R2's presigned-POST endpoint."""
